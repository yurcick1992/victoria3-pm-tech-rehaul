...
| **the PROFIT-WAGE SHARE** | **0.30** — the second term of a PREDICTION only: `W = 1.19 × the normal-rate bill + 0.30 × the building's own profit`, so `P = (R − I − 1.19·Wm) ÷ 1.30` and a designed margin is damped by 1/1.30 before any price moves. ⚠⚠ It is a REDUCED-FORM stand-in for the building's own `salary_rate`, which a country-level wage cannot see — with that rate in hand the coefficient collapses to 0.019 (F152 §9). Never quote it as how the game computes a wage |
| **`salary_rate`** | ⭐ THE BUILDING'S OWN wage rate, in a melted save's building record — the thing the two coefficients above exist to guess. Beside it sit **`goods_sales`** and **`goods_cost`**, revenue and inputs at MARKET prices, which make the margin repair unnecessary. ⚠ None of the three is in the save SUMMARY yet; `tools/building_ledger.mjs` reads them from a melt |
