...
| **the WAGE PREMIUM** | ⚠ **THERE IS NONE — it is 1.0** (F152 §10). `wage × wage units × staffed levels` reproduces the exact wage bill to ~1% in the median from 1857 on. The 1.52× and the later `1.19 + 0.30 × profit` were both artefacts of approximating revenue; the constant survives only as a named knob and to reproduce those retired readings |
| **`goods_sales` / `goods_cost`** | ⭐ revenue and inputs at MARKET prices, per building, in the save — and in the SUMMARY from **v9**. With them the margin needs no price repair and the wage needs no model: `wages = goods_sales − goods_cost − profit` |
