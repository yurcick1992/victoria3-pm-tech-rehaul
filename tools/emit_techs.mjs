...
// ⚠⚠ STALENESS GUARD. `build.ps1` calls THIS file but never `tech_tree_spec.mjs`, so the tree data is
// not regenerated by a build and can fall behind the config it was derived from. Editing a recipe in
// the balance UI is harmless — each tier's `tech` lives in mod_config.json and the builder reads it
// directly — but ADDING a tier there produces a tier no technology knows about, and nothing would say
// so. Same guard, and the same reasoning, as bundle_ui.mjs refusing to bundle stale ui/*.js.
{
  const opts = join(REPO, 'config', 'tech_tree_options.json');
  const cfg = join(REPO, 'config', 'mod_config.json');
  if (statSync(opts).mtimeMs < statSync(cfg).mtimeMs) {
    throw new Error(
      `config/tech_tree_options.json is OLDER than config/mod_config.json, so the tech tree was ` +
      `derived from a config that has since changed. A tier added since would have no technology and ` +
