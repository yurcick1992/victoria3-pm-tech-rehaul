...
// ===================================================================================================
// 6. DEFINES — the ahead-of-time penalty (partial override; the one folder that allows it), plus any
//    CONFIG-DRIVEN AI defines (`ai_defines`: map NAI key → value; user-ruled 2026-08-25 for the
//    investment-hoard levers — absent or empty emits no NAI block at all, so the canonical build is
//    unchanged unless the config opts in)
// ===================================================================================================
const AI_DEFINES = CFG.ai_defines || {};
write('common/defines/01_pm_rehaul_defines.txt',
  ...
  '}\n' +
  (Object.keys(AI_DEFINES).length
    ? 'NAI = {\n' + Object.entries(AI_DEFINES).map(([k, v]) => `\t${k} = ${v}\n`).join('') + '}\n'
    : ''));
