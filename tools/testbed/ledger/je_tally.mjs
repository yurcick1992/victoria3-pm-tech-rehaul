...
for (const rel of runs) {
  ...
  for (const y of YEARS) { const S = at(y); if (!S) continue; const row = ['GBR', 'FRA', 'USA', 'GER', 'NGF', 'PRU', 'RUS'].map(t => { const c = S.countries?.[t]; if (!c) return `${t} —`; const n = Array.isArray(c.technologies_held) ? c.technologies_held.length : (typeof c.technologies === 'number' ? c.technologies : '?'); return `${t} ${n}`; }).join(' · '); console.log(`   technologies held at ${y}: ${row}`); }
