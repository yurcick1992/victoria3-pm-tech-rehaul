...
# ---- ERA presets: solved, not derived from vanilla history ----------------------------------------
# config/era_presets.json is produced by tools/era_scenarios.mjs (the Node solver), which fits the mod's
...
# it: in the real game it is minted into the treasury, and this model has no treasury, so it is a good with
# producers and no consumer. Left in the country presets it would do exactly the same damage on the
# REFERENCE side — 82 gold mines and 31 gold fields across the shipped presets, all selling into a market
# with no buyer, pinning gold at the 25% floor and pulling their workforce into every derived figure.
# ⚠ Done HERE rather than in the three places presets are built, because a filter that has to be remembered
# in three code paths is a filter that will be missed in one. Handles both shapes: the country and
# placeholder presets are ordered dictionaries, the era presets arrive from JSON as PSCustomObjects.
$goldBld = @('building_gold_mine', 'building_gold_field')
$goldStripped = 0
foreach ($p in $out) {
    foreach ($field in @('buildings', 'pms', 'throughput')) {
        $v = $null
        if ($p -is [System.Collections.IDictionary]) { if ($p.Contains($field)) { $v = $p[$field] } }
        elseif ($p.PSObject.Properties[$field]) { $v = $p.$field }
        if ($null -eq $v) { continue }
        foreach ($g in $goldBld) {
            if ($v -is [System.Collections.IDictionary]) { if ($v.Contains($g)) { $v.Remove($g); $goldStripped++ } }
            elseif ($v.PSObject.Properties[$g]) { $v.PSObject.Properties.Remove($g); $goldStripped++ }
        }
    }
    # …and gold as a TRADED good, which would re-introduce the same one-sided order book through the
    # trade columns even with every producer gone.
    foreach ($field in @('nonbuy', 'nonsell', 'prices')) {
        $v = $null
        if ($p -is [System.Collections.IDictionary]) { if ($p.Contains($field)) { $v = $p[$field] } }
        elseif ($p.PSObject.Properties[$field]) { $v = $p.$field }
        if ($null -eq $v) { continue }
        if ($v -is [System.Collections.IDictionary]) { if ($v.Contains('gold')) { $v.Remove('gold'); $goldStripped++ } }
        elseif ($v.PSObject.Properties['gold']) { $v.PSObject.Properties.Remove('gold'); $goldStripped++ }
    }
}
Write-Host ("  gold stripped from presets: {0} entrie(s) removed (mines, fields, and gold as a traded good)" -f $goldStripped)
