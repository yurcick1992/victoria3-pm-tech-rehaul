...
if (-not $isAlt) {
    ...
    # lists), not the scenarios: those ride ui/presets.js through extract_presets.ps1. A build-time
    # copy like everything else in this file — after 'node tools/era_inverse.mjs --write', rebuild.
    # Absent file => null and the UI disables the button.
    $invPath = Join-Path $repo 'config\era_inverse.json'
    $invJson = 'null'
    if (Test-Path $invPath) {
        $inv = Get-Content $invPath -Raw -Encoding UTF8 | ConvertFrom-Json
        if ($inv.PSObject.Properties['recipes']) {
            $invSub = [ordered]@{ recipes = $inv.recipes; mandate = $inv.mandate; ladder_yields = $inv.ladder_yields }
            $invJson = $invSub | ConvertTo-Json -Depth 10 -Compress
        }
    }
