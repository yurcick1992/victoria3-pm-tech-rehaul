...
# input-output break-even output price (BE%) using:
...
# with no optional PMs toggled on. Checked against the tier band in BALANCE_FRAMEWORK.md §3.
#
# Inputs (file1 = tier map, file2 = concatenated vanilla+mod defs, vanilla first so mod
# overrides). Driven by lint.sh. Exit 1 if any in-scope building is outside its band.
...
# ---- file1: tier map (pm  tier  target_be) ----
FNR==NR { if ($1 ~ /^pm_/) { tier[$1]=$2+0; target[$1]=$3+0 } next }
