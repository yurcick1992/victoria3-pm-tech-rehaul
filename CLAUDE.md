...
tools/                  dev tooling — NOT shipped in the mod
  ...
  emit_research_events.mjs  THE INDUSTRY-DRIVEN RESEARCH EVENTS (ROADMAP step 2) — called by build.ps1, which
                        ...
                        an event on them could never fire. Current output: 122 technologies (82 industry,
                        40 war) → 366 journal entries, 122 bars, 115 script values, 1103 loc keys
...
- **Manual start overrides** live in `config/start_exceptions.json`: rules targeting a vanilla base
  ...
  industry id THROWS) or `remove` (delete the factory). Most
  specific scope wins. The rules list is currently **EMPTY**: the 1836 steamer seed (§10.59,
  user-ruled 2026-08-16) was **REVERTED the same day** — it existed only to feed the abnormal
  from-start steam-port construction, which F66 traced to the engine provisioning every overseas
  state once per port building TYPE; fix the ports, not the symptom. The `force_industry_tier`
  machinery stays, documented and tested. To author
...
- **New-economy industries (clone-and-swap) — power / port / railway.** These vanilla buildings carry
  ...
  **`no_mass_be: true`** (all three — excluded from the linter ladder and, in the UI, locked-by-default so
  ...
  blip). See §10.60.2 for the three regeneration traps (payback_census --write would un-divide the cost).
