...
- **Manual start overrides** live in `config/start_exceptions.json`: rules targeting a vanilla base
  ...
  specific scope wins. The shipped rules are the **1836 steamer seed** (§10.59, user-ruled
  2026-08-16): one yard per screw_frigate holder (GBR Home Counties 5 / FRA Brittany 4 / USA
  Virginia 2 / BEL Flanders 1 / PRU West Prussia 1 = 13 levels) forced onto `building_shipyard_metal`,
  against a vanilla start of 105 clipper levels and zero steamer supply — every recipient holds the
  unlocking tech at start (screw_frigate is a tier-1-tech starting invention), so L14 stays clean. To author
