...
config/era_inverse.json     GENERATED by tools/era_inverse.mjs --write and COMMITTED: the INVERSE-SOLVE
                        experiment's artifact (BALANCE_FRAMEWORK §10.65) — mandated per-era prices, the
                        recipe book derived from them, and the six seeded feasibility scenarios. A design
                        STUDY, read by nothing in the build or pipeline; the canonical config is untouched
