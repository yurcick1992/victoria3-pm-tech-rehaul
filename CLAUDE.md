...
config/measured_base_wages.json GENERATED by `tools/measure_wage_share.mjs --write-wages` and COMMITTED: the country
                        ...
                        ⚠ It is NOT what buildings pay: apply `lib_wage_model.WAGE_PREMIUM` (1.19) AND the second term,
                        `PROFIT_WAGE_SHARE` (0.30 × the building's own profit) — F152 §8. ⚠ It is
...
tools/                  dev tooling — NOT shipped in the mod
  ...
  ladder_options.mjs    ⭐⭐ THE LADDER OPTION TABLE — what a candidate (A, B, cost, in0) does to the THREE things that
                        ...
                        and charged the art academy almost nothing. ⭐ And THE WAGE ANSWERS BACK (F152 §8): profit is the
                        closed form `(O − I − 1.19·Wm) ÷ 1.30`, so a designed margin is damped before any price moves.
                        At GBR@1920 on the canon's ladder era 0 goes to **−£75/level/wk (median)**, the frontier reads
                        **127–163%** where a flat wage said 211–519%, and the wage share of cost RISES up the ladder
                        (35/35/41/49%) where a fixed wage made it fall. Then F97's DEATH TEST (a rung two
  ...
  era0_solvency.mjs     ⭐⭐ THE ERA-0 SOLVENCY CENSUS — what an input penalty does to the 1836 map at the prices the
                        ...
                        measured normal rate at the date × the 1.19× premium, in place of the seven-entry hardcoded table it
                        carried — which was derived from F92’s identity, the mixed-units ratio F150 condemned — and THE WAGE
                        ANSWERS BACK (F152 §8), so every case is the closed form `(O − I − Wn) ÷ 1.30` and a loss-making rung
                        is charged LESS wage, which is the engine’s own lower-wages rule. Vanilla’s own 1836 recipe reads
                        **21 insolvent cells (30%) at a median +14%** against the old hardcoded table’s **22 (31%) at +21%**.
                        `--profit-wage-share 0` restores the fixed-wage reading. ⚠ **F140’s census FIGURES are superseded**;
  ...
  lib_wage_model.mjs    ⭐⭐⭐ THE WAGE MODEL AND THE REPAIRED MARGIN IDENTITY — ONE implementation, because the
                        ...
                        **£/employee/week = a summary's `base_wage` ÷ 10,000** (the engine's own unit, stated in
                        ...
                        measured normal rate × the measured **1.19×** premium — ⭐⭐ **`predictProfit()`**, which is
                        the closed form the SECOND wage term forces: the bill is `1.19 × Wm + 0.30 × the building's own
                        profit`, because the engine raises a wage where the building can afford to, so
                        `P = (R − I − 1.19·Wm) ÷ 1.30` and **a recipe cannot set a margin** (F152 §8) —
  ...
  fit_wage_model.mjs    ⭐⭐ WHERE THE WAGE MODEL'S TWO COEFFICIENTS COME FROM (FINDINGS F152 §8). Read-only.
                        ...
                        so the design matrix is not clean enough to trust coefficient by coefficient; (2) ⭐ the
                        TWO-TERM fit that ships, `W = 1.19 × the normal-rate bill + 0.30 × the building's own profit`,
                        identified (collinearity 0.76) and stable across all seven countries; (3) which form predicts
                        a building's own profit best — the test that decides it (26.5% median error against the flat
                        premium's 38.7%, bias gone). `node tools/fit_wage_model.mjs <session/run> …`
