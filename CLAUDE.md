...
config/mod_config.canon-dams.json ⭐ THE DAM BOOK (ROADMAP step 6, BALANCE_FRAMEWORK §10.89, user-ruled 2026-09-26) + its tree twin (the
                        ...
                        granted to ten majors, 2-month surveys, cost × 0.05 — probe builds ONLY). First century run: schedule
                        canon_dams_n1.json. NOT canonical. Un-ignored with its twin
...
tools/                  dev tooling — NOT shipped in the mod
  ...
  lib_dams.mjs          ⭐ THE HYDRO-DAM DERIVATION — ONE implementation (BALANCE_FRAMEWORK §10.89): project → stages (≤ 10,000 points each,
                        all opening at the survey), points per MW = 0.4 × (power-plant points ÷ 50) × 1.2 × m × (MW per part ÷ 560)^−0.2
  ...
  emit_dams.mjs         THE DAMS, EMITTED — called by build.ps1 (throws); `dams.enabled` absent/false → nothing. Writes: the building group
                        bg_pmr_hydro_dams (government-funded), the unique stage buildings building_dam_<id>_<k> (anchor-province `possible`,
                        the Victoria Terminus pattern), their methods, the survey modifier / scripted effect / PLAYER decision (ai_chance 0)
                        / stop decision / counter journal entry, the start and on_building_built effects, the quarterly AI DRIVER
                        (on_monthly_pulse_country, 90-day cooldown: one survey and one stage per quarter within GDP-scaled slots — FINDINGS
                        F168: the AI takes every visible decision in one pass and never queues these buildings itself), the yearly
