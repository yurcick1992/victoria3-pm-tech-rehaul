...
soft > 1.0 · U* soft < 1.0 · H aim < 1×. ⭐⭐ **W CARRIES HARD BOUNDS SINCE 2026-09-19 (user-ruled), AND H DOES NOT** — the user: *"the hard boundaries were meant to be for W. H is hardly comparable with vanilla anyway"*, which is a measurement: vanilla’s own end-state H runs **0.56–1.93×** between its sixteen seeds (pool 0.40–2.74×) where its W runs **0.86–1.08×** (pool 0.89–1.10×), so only W can carry a vanilla-derived line. The rule is [the 2.5th percentile × 0.9, the 97.5th × 1.1], DERIVED per invocation: **ceiling world 1.19× / pool 1.21×, above it = FULL DEPEASANTATION = broken by runoff** (it was a SOFT flag at > 1.0 until now). ⚠⚠ The ruled FLOOR (0.78 / 0.80×) sits INSIDE the register’s own W aim of 0.6–0.95 and marks **every book we have** broken by stall (measured with `--w-hard both`: the canon, canon-lvl-a19b17, canon-c195-in12 and canon-a205-gm, one intact run in five books), so the shipped default is **`--w-hard aim`** — the ruled ceiling, and the floor taken from the register’s own aim floor × 0.9 = **0.54×**, which binds almost nowhere. WHICH FLOOR STANDS IS THE USER’S (BALANCE_FRAMEWORK §10.86.1). **HARD — CAPITAL ABUNDANCE (agreed 2026-09-17 13:30):** a shortlist member of 50M+ people with U* under 5% in FIVE or
