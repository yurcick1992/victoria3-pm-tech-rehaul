...
**The arm.** `config/mod_config.canon4-art.json` = canon4-je (canon-4rung + the ruled research events, F102 — economically
the canon) with the art academy alone at **A = 3** (B stays 1.5): fine art **5 / 15 / 45 / 135** a level (canon
5 / 10 / 20 / 40; rung 3 = 27× rung 0), inputs unchanged, building_cost capacity-priced ×3^k (**400 / 1,200 / 3,600 /
10,800** points), ai_value 1000×3^era (1000 / 3000 / 9000 / 27000), target_be **40 / 20 / 10 / 5%**. Generated by
`make_ab_config.mjs --A 2.0 --B 1.5 --ai-steep glass,tooling:3 --A-for art_academy:3 --divisor 0.000125` (the new
`--A-for` gives one industry its own output ratio; the divisor is PINNED because the generator's auto value follows the
dearest rung and a 10,800-point academy would have re-scored every building's private-pool desire), research_events
transplanted from canon4-je; verified: every other industry byte-identical, lints and solvency pass, the tree twin is
canon4-je's. Schedule `tools/testbed/schedules/canon4_art_n3.json` (n=3, ~8 h) — NOT launched, awaiting the go-ahead.

**What the arithmetic already says, before a run.**
- The engine's price floor is 25% of base. Rung 0 breaks even at 40% and DIES at the floor; rungs 1–3 (20 / 10 / 5%)
  survive at the floor on goods cost alone — so whether the old academies empty is decided by the wage/SoL layoff (F97),
  not by price. That is the same death mechanism the whole ladder rests on, at its most extreme ratio (27:1).
- F19 measured a large market's entire leisure budget buying ~96 fine art a week at base price; one e2 level now makes 45
  and one e3 level 135, so the floor is reached with a handful of levels and the price shock is certain. The open question
