...
|---|---|---|---|
| The **four-rung structure** (canon4v: rungs, names, gates, recipes, staffing, icons, secondary groups, building blocks) | `common/buildings/*.txt`, `common/production_method_groups/*.txt`, `common/production_methods/*.txt`, `localization/english/**` | `make_tier4_config.mjs` | Reads the game and `tools/lib_tier4_spec.mjs` only (BALANCE_FRAMEWORK §10.72). A patch that renames a method, re-gates it or adds a fifth main method changes the ladder; the tool throws on more than four |
| The **four-rung tech tree** (vanilla eras, names, prerequisites + ERA_MOVES + 3 minted since 2026-09-13, motor's dropped under the era rule) | `common/technology/technologies/*.txt`, `localization/english/**` | `make_tier4_techs.mjs` | The twin is rebuilt from the game; `reEra` marks the one moved technology (vulcanization) so `emit_techs.mjs` patches the vanilla file |
| The **default build** (`config/mod_config.json` = **canon-c19-in12-elec** since 2026-09-20) | everything the two rows above read | `build.ps1` (no `-Config`) | The deployed mod is the four-rung canon. After a patch: regenerate it through the pipeline (make_tier4_config → make_tier4_techs → make_ab_config with the canon's own `_ab.command`), diff against the committed copy, copy over. The six-rung machinery (era pipeline, `tech_tree_spec --write`) runs ONLY with `MOD_CONFIG=config/mod_config.six_rung.json`; it throws otherwise. |
| **The wage model and the margin repair** (BALANCE_FRAMEWORK §10.87, FINDINGS F152) | `common/pop_types/*.txt` (`wage_weight`) and `common/defines/00_defines.txt` (`POP_SIZE_PACKAGE` — the DENOMINATOR of the engine's wage rate, which is weekly £ per that many employees, stated on `NORMAL_WAGE_RATE_FALLBACK` / `MINIMUM_WAGE_RATE`) | `tools/lib_wage_model.mjs` | Both read LIVE and both THROW if the key moves (fewer than 10 pop types with a weight, or no `POP_SIZE_PACKAGE`), so a patch cannot leave the wage silently rescaled — unlike the four static mirrors of `wage_weight` listed under *Testbed couplings*. ⚠ **`config/measured_base_wages.json` is a MEASURED snapshot of VANILLA and does NOT re-derive**: a patch that changes wages or the starting economy makes it stale, silently. Regenerate with `tools/measure_wage_share.mjs --write-wages` against a post-patch vanilla baseline — which needs one to exist first. ⚠ So are the wage bill's **two fitted coefficients** — `WAGE_PREMIUM` 1.19 and `PROFIT_WAGE_SHARE` 0.30 (F152 §8): re-measure both with the same tool if the `BUILDING_*_WAGES_*` or `BUILDING_PROFIT_TARGET_TO_*_WAGES` defines move, since the second coefficient IS that wage-raising rule. |
...
| **Technology files** (whole-file replace, **only when they carry something**) | `common/technology/technologies/{10_production,20_military,30_society}.txt` + `common/scripted_effects/00_starting_inventions.txt` (+ `common/technology/eras/*` read for era costs) | `emit_techs.mjs` (via `build.ps1`) | 10 always owned (era moves + the `aniline` prereq swap); 20/30 owned **only** when they carry era moves or a `tech_ai_weight_mult` ≠ 1 (at the ruled 1/1/1 defaults, 30_society is NOT emitted). Every transform asserts its exact match count and **throws** on a no-op, so a patch that moves an anchor fails the build loudly rather than shipping a silent miss. The 1836 grant re-derives from `start_baseline.json` + vanilla's own named grants each build. |
...
| **Company chain extension** (whole-file replace of ALL changed company files — today all 22) | **every** `common/company_types/*.txt`; `common/buildings/*.txt` (plural `aliases` resolution); `common/modifier_type_definitions/*.txt` (existence check + anchor bodies to copy); `localization/<lang>/modifiers_l_<lang>.yml` per language (anchor modifier loc lines) | `emit_companies.mjs` (via `build.ps1`) | Expands every company reference to a tiered industry into the whole chain (BALANCE_FRAMEWORK §10.64). Reads vanilla LIVE, so a patch's **new or retuned companies flow through on rebuild** — including any new company naming one of our anchor keys. Throws on a no-op transform, on a mapped key in an unhandled context (a new trigger idiom around `is_building_type` would fail the build loudly, not ship a company quietly locked to rung 1), and on a missing anchor modifier type to copy. **Re-check after a patch:** (a) the build line `emit_companies: N files … M companies …` — the counts moving on an unchanged config means vanilla's company set moved; (b) a patch adding a NEW per-building modifier suffix to a company `prosperity_modifier` on our industries is expanded automatically but needs its anchor modifier type to exist (throws if not); (c) the 5-value charter `type` enum and the hardcoded formation gates (tech-for-one-listed-type, 5 levels in one state) are ENGINE-side — re-verify against patch notes if companies misbehave. |
| **Finish boost** (`research_events.finish_boost`, 2026-09-23, FINDINGS F160) — patches the `ai_weight` of every JE-covered technology; **owns `20_military.txt` and `30_society.txt` whenever they hold a covered technology** (regenerated from vanilla each build) | `common/technology/technologies/*.txt` as the ENGINE loads them (vanilla overlaid by the mod's same-named files + additive ones): each technology's `era`, `category`, `can_research`; `common/technology/eras/*.txt` (`technology_cost`); `TECH_AHEAD_OF_TIME_PENALTY_FACTOR` (our defines, else `common/defines/00_defines.txt`) | `emit_tech_finish.mjs` (via `build.ps1`) | Re-derives the penalty values and the thresholds every build, so an era move or a new technology needs nothing by hand. **Re-check after a patch:** (a) a technology gaining a CONDITIONAL `can_research` makes the build THROW by design — decide whether the penalty counts it; (b) the engine facts it rests on are ENGINE-side and were measured on 1.13: the cost formula `era_cost × (1 + F × Σ)`, `can_research = no` skipped by the penalty, and a grant reaching the cost acquiring the technology on the spot — re-run the diagnostic probe (`finish_boost.diag: true`, `tools/testbed/ledger/finish_probe.mjs`) if the patch notes touch research; (c) vanilla's heaviest `ai_weight` (101.5 in 1.13) against `finish_boost.add` 10000. |
...
- **2026-08-23** — **The company chain extension (§10.64) adds four vanilla couplings, all read LIVE
  per build** by `emit_companies.mjs`: every `common/company_types/*.txt` (whole-file-owned when
  changed — today all 22), building plural `aliases`, `common/modifier_type_definitions/` (anchor
  throughput-modifier bodies are copied verbatim for the invented tier keys — vanilla hand-enumerates
  these types, they are NOT auto-generated per building), and each language's `modifiers_l_<lang>.yml`
  (anchor loc lines re-pointed at tier keys). Every transform asserts its match count and throws on a
  no-op or an unhandled reference, so a patch that reshapes company script fails the build loudly. See
  the Automated table row above.
