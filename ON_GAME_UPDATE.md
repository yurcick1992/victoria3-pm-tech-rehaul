...
| The **default build** (`config/mod_config.json` = canon4v since 2026-09-05) | everything the two rows above read | `build.ps1` (no `-Config`) | The deployed mod is the four-rung canon. After a patch: regenerate the canon through the pipeline (make_tier4_config → make_tier4_techs → make_ab_config --A 2.0 --B 1.5 --suffix canon4v-hai3 --ai-steep <all 17>:3 --divisor 0.000125 --ai-defines <the five of §10.75>), diff against the committed copy, copy over. The six-rung machinery (era pipeline, `tech_tree_spec --write`) runs ONLY with `MOD_CONFIG=config/mod_config.six_rung.json`; it throws otherwise. |
