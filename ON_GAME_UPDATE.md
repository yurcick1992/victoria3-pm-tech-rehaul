...
| **Finish boost** (`research_events.finish_boost`, 2026-09-23, FINDINGS F160) — appends `if = { limit = { has_technology_progress = { technology = X progress >= 0.99 } } add = 10000 }` to the `ai_weight` of EVERY researchable technology; **owns all three vanilla technology files** (regenerated from vanilla each build) | `common/technology/technologies/*.txt` as the ENGINE loads them (vanilla overlaid by the mod's same-named files + additive ones): each technology's block, `ai_weight`, `can_research` | `emit_tech_finish.mjs` (via `build.ps1`) | Reads vanilla live, so a patch's new or re-weighted technologies are patched on rebuild. **Re-check after a patch:** (a) a technology gaining a CONDITIONAL `can_research` makes the build THROW by design; (b) ENGINE-side facts measured on 1.13 — `has_technology_progress`'s `progress` is a fraction of the PENALISED cost and the `>=` form parses (F160: 219/219), and a grant reaching the cost acquires the technology on the spot; if the patch notes touch research or the AI, re-verify with the FORCE TEST (`finish_boost.force_test`, schedule `finish_boost_p45_force_ab.json`, reader `tools/testbed/ledger/finish_force.mjs` — 2 short runs; 1.13 read 52/52 ON against 0/42 OFF); (c) vanilla's heaviest `ai_weight` (101.5 in 1.13) against `finish_boost.add` 10000. |
