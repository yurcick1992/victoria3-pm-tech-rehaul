...
| The **four-rung tech tree** (vanilla eras, names, prerequisites + ERA_MOVES + 4 minted) | `common/technology/technologies/*.txt`, `localization/english/**` | `make_tier4_techs.mjs` | The twin is rebuilt from the game; `reEra` marks the one moved technology (vulcanization) so `emit_techs.mjs` patches the vanilla file |
